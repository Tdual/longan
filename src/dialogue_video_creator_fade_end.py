...
class DialogueVideoCreatorFadeEnd:
    def __init__(self):
        pass
    ...
    def create_dialogue_slide(self, image_path, audio_infos):
        """対話形式の音声を持つスライドから動画クリップを作成（終了フェード版）"""
        # 画像クリップを作成
    ...
    def create_dialogue_video(self, image_paths, dialogue_audio_info, output_path="dialogue_fade_end_output.mp4", fps=24):
        """対話形式の動画を作成（終了フェード版）"""
        clips = []
        ...
        for i, image_path in enumerate(image_paths):
            ...
            print(f"スライド {i+1} の動画クリップを作成中（終了フェード版）...")
        ...
        # 動画を出力
        print(f"終了フェード版動画を出力中: {output_path}")
        final_video.write_videofile(
            output_path,
            fps=fps,
            codec='libx264',
            audio_codec='pcm_s16le',  # PCMコーデック
            temp_audiofile='temp-audio-fade-end.wav',
            remove_temp=True,
            verbose=False,
            logger='bar'
        )
