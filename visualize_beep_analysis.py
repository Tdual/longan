...
def create_visualization():
    ...
    fig.suptitle('4秒付近のビープ音異常検出 - 視覚的分析', fontsize=16, fontproperties='DejaVu Sans')
    ...
    ax1.axhline(y=80, color='yellow', linestyle='--', alpha=0.7, label='ビープ音閾値(80dB)')
    
    # 4秒付近を強調
    ax1.axvspan(3, 5, alpha=0.2, color='red', label='4秒付近（異常範囲）')
    ax1.axvline(x=4, color='red', linestyle='-', alpha=0.8, linewidth=3, label='4秒（中心）')
    
    ax1.set_ylabel('dB値', fontproperties='DejaVu Sans')
    ax1.set_title('1. ビープ音帯域（300-400Hz）の音圧レベル', fontproperties='DejaVu Sans')
    ...
    ax2.set_ylabel('周波数 (Hz)', fontproperties='DejaVu Sans')
    ax2.set_title('2. ビープ音の主要周波数の変化', fontproperties='DejaVu Sans')
    ...
    ax3.set_xlabel('時間 (秒)', fontproperties='DejaVu Sans')
    ax3.set_ylabel('dB値', fontproperties='DejaVu Sans')
    ax3.set_title('3. 全体音圧とビープ音帯域の比較', fontproperties='DejaVu Sans')
    ax3.grid(True, alpha=0.3)
    ax3.legend()
    ax3.set_ylim(0, 140)
    
    plt.tight_layout()
    plt.savefig('/Users/tdual/Workspace/longan/beep_analysis_visualization.png', dpi=300, bbox_inches='tight')
    plt.close()
    
    # 4秒付近の詳細グラフ
    fig2, (ax4, ax5) = plt.subplots(2, 1, figsize=(15, 8))
    fig2.suptitle('4秒付近の詳細分析（3-5秒）', fontsize=16, fontproperties='DejaVu Sans')
    ...
    ax4.annotate(f'最高: {max_db:.1f}dB', xy=(max_time, max_db), xytext=(max_time+0.2, max_db+5),
                arrowprops=dict(arrowstyle='->', color='red'), fontproperties='DejaVu Sans')
    ax4.annotate(f'最低: {min_db:.1f}dB', xy=(min_time, min_db), xytext=(min_time+0.2, min_db-5),
                arrowprops=dict(arrowstyle='->', color='blue'), fontproperties='DejaVu Sans')
    
    ax4.set_ylabel('dB値', fontproperties='DejaVu Sans')
    ax4.set_title('4. 3-5秒の詳細dB値変化', fontproperties='DejaVu Sans')
    ...
    ax5.set_xlabel('時間 (秒)', fontproperties='DejaVu Sans')
    ax5.set_ylabel('周波数 (Hz)', fontproperties='DejaVu Sans')
    ax5.set_title('5. 3-5秒の詳細周波数変化', fontproperties='DejaVu Sans')
    ax5.grid(True, alpha=0.3)
    ax5.legend()
    
    plt.tight_layout()
    plt.savefig('/Users/tdual/Workspace/longan/beep_detail_4sec.png', dpi=300, bbox_inches='tight')
    plt.close()
...
print(f"\n✅ グラフを保存しました:")
print(f"📊 全体分析: /Users/tdual/Workspace/longan/beep_analysis_visualization.png")
print(f"🔍 4秒詳細: /Users/tdual/Workspace/longan/beep_detail_4sec.png")
